...
function(add_windows_deploy_target)
  # BC3: dropped `AND TARGET test_bitcoin` from the guard. The installer no longer
  # bundles the test suite, so there is no need to build it either: with this the
  # `deploy` target works with -DBUILD_TESTS=OFF and the Windows build gets much shorter.
  if(MINGW AND TARGET bitcoin AND TARGET bitcoin-qt AND TARGET bitcoind AND TARGET bitcoin-cli AND TARGET bitcoin-tx AND TARGET bitcoin-wallet AND TARGET bitcoin-util)
    ...
    generate_setup_nsi()
    # BC3: the build produces vanilla names (bitcoin-qt.exe, bitcoind.exe...). The strip
    # leaves them in release/ ALREADY RENAMED to the BitcoinIII names, which are the very
    # ones GenerateSetupNsi.cmake hands to the .nsi. The two files go together.
    add_custom_command(
      OUTPUT ${PROJECT_BINARY_DIR}/bitcoinIII-win64-setup.exe
      COMMAND ${CMAKE_COMMAND} -E make_directory ${PROJECT_BINARY_DIR}/release
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-qt> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII-qt${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoind> -o ${PROJECT_BINARY_DIR}/release/bitcoinIIid${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-cli> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII-cli${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-tx> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII-tx${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-wallet> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII-wallet${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${CMAKE_STRIP} $<TARGET_FILE:bitcoin-util> -o ${PROJECT_BINARY_DIR}/release/bitcoinIII-util${CMAKE_EXECUTABLE_SUFFIX}
      COMMAND ${MAKENSIS_EXECUTABLE} -V2 ${PROJECT_BINARY_DIR}/bitcoinIII-win64-setup.nsi
      VERBATIM
    )
